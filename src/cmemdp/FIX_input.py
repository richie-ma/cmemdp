...
class quotes:
    ...
    def order_book(data, security, level, consolidate=True):
        ...
        mbp_quotes = mbp_quotes.astype({'Update': 'int', 'Qty': 'int', 'Seq': 'float',
                                        'MsgSeq': 'float', 'Ord': 'int', 'PX': 'float',
                                        'PX_depth': 'int'})

        # Subseting the outright messages and implied messages

        mbp_quotes['Side'] = np.where(mbp_quotes['Side'] == 'E', 0, np.where(
            mbp_quotes['Side'] == 'F', 1, mbp_quotes['Side']))
        ...
        message_outright = message_outright.sort_values(
            'Seq', ignore_index=True)
        ...
        message_outright['qty_diff'] = np.where(message_outright.groupby(['Side', 'PX'])['Update'].shift(1) != 2,
                                                message_outright['Qty'] - message_outright.groupby(['Side', 'PX'])[
            'Qty'].shift(1),
            message_outright['Qty'])

        message_outright['ord_diff'] = np.where(message_outright.groupby(['Side', 'PX'])['Update'].shift(1) != 2,
                                                message_outright['Ord'] -
                                                message_outright.groupby(['Side', 'PX'])[
            'Ord'].shift(1),
            message_outright['Ord'])

        message_outright.loc[message_outright['Update'] == 2,
                             'qty_diff'] = message_outright.loc[message_outright['Update'] == 2, 'Qty'] * -1
        message_outright.loc[message_outright['Update'] == 2,
                             'ord_diff'] = message_outright.loc[message_outright['Update'] == 2, 'Ord'] * -1
        ...
        if 'Y' in message_outright['Implied'].unique():
            ...
            message_implied['qty_diff'] = np.where(message_implied.groupby(['Side', 'PX'])['Update'].shift(1) != 2,
                                                   message_implied['Qty'] - message_implied.groupby(['Side', 'PX'])[
                'Qty'].shift(1),
                message_implied['Qty'])

            message_implied.loc[message_implied['Update'] == 2,
                                'qty_diff'] = message_implied.loc[message_implied['Update'] == 2, 'Qty'] * -1
            ...
            message_implied = message_implied.sort_values(
                'Seq', ignore_index=True)
        ...
        def book_reconstruction_main(quotes, level=None, conso_quotes=False):
            ...
            bid_rank = pd.pivot_table(bid, index=['Seq'], columns=[
                'PX_depth'], values=['PX'])
            ...
            offer_rank = pd.pivot_table(offer, index=['Seq'], columns=[
                'PX_depth'], values=['PX'])
            ...
            offer_price = - \
                offer.sort_values('Seq')['PX'].to_numpy()  # descending
            ...
            order_book = pd.pivot_table(quotes, index=quotes.columns[0:5], columns=[
                'Side', 'PX_depth'], values=['PX', 'Qty', 'Ord'])
        ...
        if message_outright is not None and message_implied is None:
            ...
            LOB_outright = None
            print("No outright orders and the consolidated limit order book is the same as the implied limit order book\n")
            ...
            if consolidate:
                ...
                conso_quotes = conso_quotes.sort_values(
                    'Seq', ignore_index=True)

                conso_quotes['Qty'] = conso_quotes.groupby(
                    ['Side', 'PX'])['qty_diff'].cumsum()
                conso_quotes['Ord'] = conso_quotes.groupby(
                    ['Side', 'PX'])['ord_diff'].cumsum()
